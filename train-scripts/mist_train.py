...
def get_ratios(ldm_stable, prev_ratio, ratio_diff, max_ratio_gap, concepts, classes, num_samples=10, num_loops=3):
    ...
    # torch.save(torch.tensor(seeds), 'seeds.pt')
    ################################################################################
    # unc_input = ldm_stable.tokenizer(
    #                     [''],
    #                     padding="max_length",
    ...
    #                     return_tensors="pt",
    #                 )
    # unc_emb = ldm_stable.text_encoder(unc_input.input_ids.to(ldm_stable.device))[0]
    ################################################################################

    ratios = []
    for idx, concept in enumerate(concepts):
        prompt = f'a headshot of {concept}'
        probs_full = []
    ...
    #                 break
                ...
                # prompt_emb[num_tokens:] = unc_emb[num_tokens:]
                images = ldm_stable(prompt_embeds=prompt_emb, negative_prompt = ['black and white, drawing, painting, unrealistic, low quality'],  num_images_per_prompt=num_samples, num_inference_steps=20, generator = g).images
                ################################################################################
...
def edit_model(ldm_stable, old_text_, new_text_, retain_text_, add=True, layers_to_edit=None, lamb=0.1, erase_scale = 0.1, preserve_scale = 0.1, with_to_k=True, num_images=1, log_file='log.txt', eos_embs=None, max_bias_diff=0.05, occupation_name=None):
    ...
    print('desired:', desired_ratios)
    # desired_ratios = [torch.tensor([0.5, 0.5])]
    weight_step = 0.1
        ...
        for layer_num in range(len(projection_matrices)):
            ...
            with torch.no_grad():
                ...
                for cnt, t in enumerate(zip(old_texts, new_texts)):
                    ...
                    ################################################################################
                    context = old_emb.detach()[final_token_idx + 1:final_token_idx + 2]
                    ################################################################################
                    values = []
                    with torch.no_grad():
                        for layer in projection_matrices:
                            ...
                            o_embs = o_embs[final_token_idx + 1:final_token_idx + 2]
                            ################################################################################
#                             print(f'O_EMBS: {final_token_idx}-{len(o_embs)-max(0,farthest-final_token_idx)}')
                            ...
                            for j, emb in enumerate(embs):
                                ...
                                u = u[final_token_idx_new[j] + 1:final_token_idx_new[j] + 2]
                                ################################################################################
